<div align="center">
  ...
  <p>Ein automatisierter Web-Scraper und Discord-Bot, der den aktuellen Speiseplan der Campus-Mensa überwacht und Änderungen in Echtzeit als Rich-Embed in einen Discord-Channel pusht.</p>
</div>
...
- **Automatisches Scraping**: Überwacht dynamisch die WordPress/AJAX-Endpoints der Mensa-Website.
- **Smart Update Detection**: Integriertes State-Management via `SHA256`-Hashing. Postet nur, wenn sich der Speiseplan oder die Angebote wirklich geändert haben.
- **Rich Discord Embeds**: 
  - Farbcodierte Darstellung je nach Hauptgericht.
  - Automatische Erkennung von veganen und vegetarischen Optionen.
  - Extraktion von Preisen (Studierende / Gäste) und Inhaltsstoffen.
...
## Setup & Deployment (Für deinen eigenen Server)

Möchtest du diesen Bot für deine Fachschaft oder deinen Kommilitonen-Server nutzen? Der Bot ist Open-Source und in 5 Minuten einsatzbereit!
...
2. Erstelle einen neuen Webhook, nenne ihn z. B. `Mensa Bot` und wähle den Ziel-Channel aus.
...
4. Secret: *Füge hier deine Discord Webhook-URL ein*.

Das war's! Der Bot läuft nun vollautomatisch im Hintergrund.

---

## Lokale Entwicklung

Falls du den Bot anpassen oder erweitern möchtest:
...
# Test-Modus ausführen (Gibt JSON & Vorschau im Terminal aus, sendet NICHTS an Discord)
...
| `--check-current` | Prüft die "Aktuelle Woche" anstelle der "Nächsten Woche". |

---

## Laufzeiten & GitHub Actions

Der Bot ist so vorkonfiguriert, dass er **alle 30 Minuten zwischen 06:00 und 18:00 Uhr deutscher Zeit** läuft (`*/30 4-16 * * *` in UTC). Dies stellt sicher, dass alle Updates erfasst werden, minimiert aber gleichzeitig den Verbrauch von GitHub Actions Freiminuten (verbraucht ca. ~700 von 2.000 monatlichen Freiminuten im Free-Tier).

## Lizenz

Dieses Projekt steht unter der **MIT License** - siehe die [LICENSE](LICENSE) Datei für Details. Feel free to use, modify and distribute!
